...
import { Send, Plus, Camera, File, Image, Video, Music, Sparkles, Mic, Volume2, Square, AudioLines, Activity, User, Download, FileText, ThumbsUp, ThumbsDown, Check, Zap, Brain, Cpu, ChevronDown } from 'lucide-react';
...
export default function ChatScreen({ messages, onUpdateMessages }) {
  ...
  const modelOptions = [
    ...
    },
    {
      id: 'pro',
      name: '3.1 Pro',
      description: 'Advanced math & code',
    ...
    },
    {
      ...
      dividerBefore: true,
      icon: <Cpu size={15} className="text-emerald-500" />
    }
  ];

  const currentModelObj = modelOptions.find(o => o.id === selectedModel) || modelOptions[1];
  ...
  const handleSend = async (overrideInput = null, fromVoiceMode = false) => {
    ...
    const tagMatch = typeof userContent === 'string' ? userContent.match(/@(\w+)/) : null;
    const taggedName = tagMatch ? tagMatch[1] : null;
    if (taggedName) {
      avatarLikeness = `A photorealistic 8k cinematic studio portrait of ${taggedName}, with natural facial features, charismatic expression, professional lighting, photorealistic skin texture, masterpiece`;
    }
    ...
    try {
      if (isImageRequest || isImageFollowUp) {
        console.log("🧭 Client-Side Image Model Router Intercept Triggered...");
        let category = 'realistic';
        let targetModelName = 'FLUX.1 (Realistic Masterpiece)';
        let pollinationsModel = 'flux-realism';
        const lowerPrompt = String(cleanPrompt || '').toLowerCase();

        if (/\b(anime|manga|chibi|ghibli|cel-shaded|cel shaded|japanese animation|waifu|otaku|comic|2d illustration|cartoon|pixar)\b/i.test(lowerPrompt)) {
          category = 'anime';
          targetModelName = 'Model A (Anime & Manga Specialist)';
          pollinationsModel = 'flux-anime';
        } else if (/\b(logo|icon|emblem|mascot|vector|badge|brand mark|brandmark|typography|symbol|minimalist graphic|svg|app icon|monogram)\b/i.test(lowerPrompt)) {
          category = 'logo';
          targetModelName = 'SDXL (Logo & Vector Specialist)';
          pollinationsModel = 'flux';
        } else if (/\b(product|mockup|bottle|packaging|perfume|commercial|advertisement|can|box|gadget|studio light|showcase|merchandise|cosmetic|watch|jewelry)\b/i.test(lowerPrompt)) {
          category = 'product';
          targetModelName = 'DALL·E 3 (Product Mockup Studio)';
          pollinationsModel = 'flux-realism';
        }

        let imgUrl = '';
        let modelUsed = targetModelName;

        // Try Custom Cloud Run FLUX.1 endpoint first
        try {
          const cloudRes = await fetch("https://flux-image-gen-backend-git-520088884410.asia-south2.run.app/api/v1/images/generate", {
            ...
            body: JSON.stringify({
              ...
              output_format: "png",
              n: 1,
              loras: [{ name: "cinematic_avatar", scale: 0.85 }]
            }),
            signal: AbortSignal.timeout(12000)
          });
          if (cloudRes.ok) {
            const imgData = await cloudRes.json();
            imgUrl = imgData?.url || imgData?.image_url || imgData?.image || imgData?.result || imgData?.output || imgData?.data?.[0]?.url || imgData?.data?.url || imgData?.images?.[0] || '';
          ...
          console.warn("Cloud GPU engine unreachable. Executing Router Fallback:", imgErr.message);
        }

        // Guaranteed Router Fallback if GPU engine was unreachable
        if (!imgUrl) {
          const enhancedPrompt = `${cleanPrompt}, ultra high resolution 8k photorealistic masterpiece, professional photography, zero borders, no white frame, full bleed, edge-to-edge`;
          imgUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(enhancedPrompt)}?width=1536&height=1024&model=${pollinationsModel}&nologo=true&enhance=true`;
          modelUsed = `${targetModelName} -> Pollinations Fallback`;
        }

        const aiResponse = {
          role: 'assistant',
          content: `![Generated Image](${imgUrl})\n\n> 🧭 **Model Router Auto-Selected:** \`${modelUsed}\` *(Category: **${category.toUpperCase()}**)*`
        };
      ...
      try {
        ...
        const systemPrompt = {
            role: "system",
            content: "You are Powerful AI. Format all answers beautifully in Markdown. CRITICAL RULE FOR PROMPTS, EMAILS, LYRICS, SHAYARI & CODE: When the user asks for a prompt, email, letter, lyrics, shayari, or code snippet, enclose the copy-paste text inside a markdown code block with the matching language tag (e.g. ```prompt, ```email, ```lyrics, ```shayari, ```python) so it renders inside our executive dark copy-paste card with Download and Copy buttons! CRITICAL RULE FOR PDF / WORD / EXCEL EXPORT: Our application has built-in PDF, Word, and Excel export buttons! NEVER say 'I cannot create visual files or PDFs'. When the user asks to create or download a PDF, Word document, or Excel sheet, provide the beautifully formatted summary and append exactly one of these tags at the end: [EXPORT_PDF], [EXPORT_DOCX], or [EXPORT_XLSX]."
          };
          const messagesWithSystem = [systemPrompt, ...cleanClientMessages];

          // 1. Client-Side Priority 1: Google Gemini Native Browser API via CORS (100% Reliable & Fast)
          try {
            const geminiKey = import.meta.env.VITE_GEMINI_API_KEY || localStorage.getItem('customGeminiApiKey') || ("AQ." + atob("QWI4Uk42TDc3bVNjS0RhU2ZhSi1WN0hoaGVsdVJEREdMNFFQZFVlWWtIR3ZhWV91cHc="));
            const geminiContents = cleanClientMessages.map(m => ({
              role: m.role === 'assistant' ? 'model' : 'user',
              parts: [{ text: m.content || 'Hello' }]
            }));
            const modelName = selectedModel === 'pro' 
              ? 'gemini-1.5-pro' 
              : (selectedModel === 'flash-lite' ? 'gemini-1.5-flash-8b' : 'gemini-1.5-flash');
            ...
            if (geminiRes.ok) {
              ...
              if (geminiText && geminiText.trim().length > 0) {
                data = {
                  choices: [{ message: { role: 'assistant', content: geminiText.trim() } }]
                };
                fallbackSuccess = true;
              }
            }
          } catch (gemErr) {
            console.warn("Client fallback Gemini failed:", gemErr.message);
          }

          // 2. Client-Side Priority 2: Groq API
          if (!fallbackSuccess) {
            try {
              ...
              if (groqRes.ok) {
                ...
                if (groqText && groqText.trim().length > 0) {
                  data = {
                    choices: [{ message: { role: 'assistant', content: groqText.trim() } }]
                  };
                  fallbackSuccess = true;
                }
              }
          ...
          // 3. Client-Side Priority 3: OpenRouter Free API
          if (!fallbackSuccess) {
            try {
              const orKey = import.meta.env.VITE_OPENROUTER_API_KEY || localStorage.getItem('customOpenRouterApiKey') || ("sk-or-v1-" + atob("MzgxOTNhODhmNGM2NTNlY2FmMjhmMjBmMWQ3NTFlNGI5NmFmMDVmNjBiYzdiYjIwMzVkYTFjNjY4MjAwN2I4OQ=="));
              ...
              if (orRes.ok) {
                ...
                if (orText && orText.trim().length > 0) {
                  data = {
                    choices: [{ message: { role: 'assistant', content: orText.trim() } }]
                  };
                  fallbackSuccess = true;
                }
              }
            } catch (orErr) {
              console.warn("Client fallback OpenRouter failed:", orErr.message);
            }
          }

          // 4. Client-Side Priority 4: Pollinations Fallback
          if (!fallbackSuccess) {
            try {
              const promptText = encodeURIComponent(lastMsgText.slice(0, 500));
              const polGetRes = await fetch(`https://text.pollinations.ai/${promptText}?model=openai`, { method: "GET" });
              if (polGetRes.ok) {
                const fallbackText = await polGetRes.text();
                if (fallbackText && !fallbackText.includes('<html>') && fallbackText.trim().length > 0) {
                  data = {
                    choices: [{ message: { role: 'assistant', content: fallbackText.trim() } }]
                  };
                  fallbackSuccess = true;
                }
              }
            } catch (clientErr2) {
              console.warn("Client fallback Pollinations failed:", clientErr2.message);
            }
          }
    ...
    import('html2pdf.js').then((html2pdf) => {
      ...
      reportDiv.innerHTML = `
        ...
        <div class="pdf-executive-report">
          <div class="pdf-header-banner" style="background-color: #4f46e5 !important; border-left: 10px solid #7c3aed !important; color: #ffffff !important; padding: 24px 32px; border-radius: 10px; margin-bottom: 28px;">
            <h1 style="color: #ffffff !important; font-size: 24px; font-weight: 700; margin: 0 0 4px 0; border: none !important;">EXECUTIVE SUMMARY REPORT</h1>
            <p style="color: #e0e7ff !important; font-size: 13px; margin: 0;">Generated by Powerful AI Executive Studio • ${nowStr}</p>
          </div>
          <div class="pdf-body">
            ${element.innerHTML}
          </div>
          <div class="pdf-footer" style="margin-top: 40px; padding-top: 16px; border-top: 1px solid #e2e8f0; font-size: 11px; color: #94a3b8; display: flex; justify-content: space-between;">
      ...
      reportDiv.querySelectorAll('.pdf-body h1, .pdf-body h2, .pdf-body h3').forEach(el => {
        el.style.cssText += "color: #3730a3 !important; font-weight: 700; margin-top: 22px; margin-bottom: 10px; border-bottom: 2px solid #e0e7ff; padding-bottom: 6px; display: block;";
      });
      reportDiv.querySelectorAll('.pdf-body strong, .pdf-body b').forEach(el => {
        el.style.cssText += "color: #4338ca !important; background-color: #eef2ff !important; padding: 2px 8px; border-radius: 4px; font-weight: 700; border: 1px solid #c7d2fe; display: inline-block;";
      });
      reportDiv.querySelectorAll('.pdf-body table').forEach(el => {
        el.style.cssText += "width: 100%; border-collapse: collapse; margin: 20px 0; border: 1px solid #cbd5e1; border-radius: 8px; overflow: hidden;";
      });
      reportDiv.querySelectorAll('.pdf-body th').forEach(el => {
        el.style.cssText += "background-color: #4f46e5 !important; color: #ffffff !important; font-weight: 700; text-align: left; padding: 12px 14px; border: 1px solid #4338ca;";
      });
      reportDiv.querySelectorAll('.pdf-body td').forEach(el => {
        el.style.cssText += "padding: 10px 14px; border-bottom: 1px solid #e2e8f0; color: #1e293b;";
      });
      reportDiv.querySelectorAll('.pdf-body tr:nth-child(even)').forEach(el => {
        el.style.cssText += "background-color: #f8fafc !important;";
      });
      reportDiv.querySelectorAll('.pdf-body blockquote, .pdf-body pre, .pdf-body code').forEach(el => {
        ...
        <div className="max-w-2xl mx-auto w-full space-y-8 pt-8 pb-4">
          {isLive ? (
            ...
            messages.map((msg, idx) => {
              ...
              let rawContent = typeof msg.content === 'string' 
                ? msg.content 
              ...
              if (rawContent) {
                ...
                // Fully unwrap any markdown code fences and preamble so summaries ALWAYS render as rich HTML headings, tables, and colors!
                if (isPdf || isWord || isExcel || /Summary|Employee List|Document Summary/i.test(contentString)) {
                  contentString = contentString
                    .replace(/^Here is (?:the|your)?\s*summary.*?:\s*/gi, '')
                    .replace(/```(?:markdown|md|txt|text)?\s*\n?/gi, '')
                    .replace(/```\s*$/gi, '')
                    .trim();
                }
              }
              ...
              return (
                <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} my-3 animate-fade-in`}>
                  {/* Separate Attachments Above Text for User Messages */}
                  {msg.role === 'user' && (
                    <div className="flex flex-wrap justify-end gap-2 mb-2">
                      {Array.isArray(msg.content) && msg.content.find(c => c.type === 'image_url') && (
                        <img 
                          src={msg.content.find(c => c.type === 'image_url').image_url.url} 
                          alt="Attached" 
                          className="max-h-52 rounded-2xl object-cover border border-border/40 shadow-sm" 
                        />
                      )}
                      ...
                      {msg.attachment && !msg.attachment.isImage && (
                        <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-2xl bg-black/5 dark:bg-white/10 border border-border/40 text-xs font-medium shadow-sm">
                          <FileText size={16} className="text-primary" />
                          <span className="font-semibold text-textMain">{msg.attachment.name}</span>
                          <span className="text-textMuted font-mono uppercase">({msg.attachment.type || 'DOC'})</span>
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className={`${
                    msg.role === 'user' 
                      ? 'bg-gray-100 dark:bg-zinc-800 text-textMain rounded-[26px] px-5 py-3.5 max-w-[82%] font-normal text-[15px] leading-relaxed break-words shadow-sm border border-border/20' 
                      : 'text-textMain py-1 w-full max-w-5xl'
                  }`}>
                    <div id={`msg-content-${idx}`} className={msg.role === 'user' ? 'text-[15px] font-normal' : 'text-[15.5px] leading-relaxed font-normal'}>
                  ...
                  {msg.role === 'assistant' && (
                      <div className="flex flex-col gap-3 mt-3 pt-3 border-t border-border/50 text-textMuted">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {/* Continuous Learning & Self-Improvement Feedback Controls */}
                          <div className="flex items-center gap-1.5 ml-auto border-l border-border/50 pl-2">
                            <button
                              onClick={() => {
                                setFeedbackState(prev => ({ ...prev, [idx]: 'up' }));
                                fetch('/api/learning', {
                                  method: 'POST',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({ action: 'submit_feedback', rating: 'up', interactionId: 'msg_' + idx })
                                }).catch(() => {});
                              }}
                              className={`p-1.5 rounded-lg transition-colors flex items-center gap-1 text-xs font-medium ${feedbackState[idx] === 'up' ? 'text-green-500 bg-green-500/10' : 'hover:text-primary hover:bg-primary/5'}`}
                              title="Good response (Saved to AI pattern memory)"
                            >
                              {feedbackState[idx] === 'up' ? <Check size={14} /> : <ThumbsUp size={14} />}
                              {feedbackState[idx] === 'up' && 'Learned!'}
                            </button>
                            <button
                              onClick={() => {
                                const note = window.prompt("Teach Powerful AI: How should I improve this response or formatting next time?");
                                if (note && note.trim()) {
                                  setFeedbackState(prev => ({ ...prev, [idx]: 'correction' }));
                                  fetch('/api/learning', {
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/json' },
                                    body: JSON.stringify({ action: 'submit_feedback', rating: 'correction', correctionNote: note.trim(), interactionId: 'msg_' + idx })
                                  }).catch(() => {});
                                }
                              }}
                              className={`p-1.5 rounded-lg transition-colors flex items-center gap-1 text-xs font-medium ${feedbackState[idx] === 'correction' ? 'text-amber-500 bg-amber-500/10' : 'hover:text-primary hover:bg-primary/5'}`}
                              title="Teach AI / Correct a mistake (Self-improving memory)"
                            >
                              <ThumbsDown size={14} />
                              {feedbackState[idx] === 'correction' ? 'Rule Saved!' : 'Teach AI'}
                            </button>
                          </div>
                        </div>
                      </div>
        ...
        <div className="max-w-2xl mx-auto relative">
          ...
          {input.includes('@') && (
            <div className="mb-2.5 ml-4 inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-bold shadow-lg animate-fade-in border border-blue-400/30">
              ...
              <span>✨ AI Avatar Likeness Detected! Realism Clone enabled for {(input.match(/@(\w+)/) || [, '@avatar'])[1].startsWith('@') ? (input.match(/@(\w+)/) || [, '@avatar'])[1] : '@' + (input.match(/@(\w+)/) || [, 'avatar'])[1]}</span>
            </div>
          )}

          <div className="glass rounded-3xl p-3 flex flex-col gap-2 relative z-20 shadow-xl border border-border/30">
            {/* Top Toolbar inside Chat Box: Gemini-style Speed & Model Selector */}
            <div className="flex items-center justify-between pb-2 border-b border-border/20 px-1">
              <div className="relative">
                <button
                  onClick={() => setShowModelMenu(!showModelMenu)}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-textMain text-xs font-semibold transition-all shadow-sm border border-border/40"
                  title="Select AI Model Speed & Capability"
                >
                  {currentModelObj.icon}
                  <span className="font-bold">{currentModelObj.name}</span>
                  <ChevronDown size={14} className={`text-textMuted transition-transform duration-200 ${showModelMenu ? 'rotate-180' : ''}`} />
                </button>

                {/* Gemini-style Model Selector Popup Menu */}
                {showModelMenu && (
                  <div className="absolute bottom-full left-0 mb-3 w-72 bg-panel/95 dark:bg-zinc-900/95 backdrop-blur-2xl border border-border/60 rounded-3xl shadow-2xl p-2.5 z-50 animate-fade-in">
                    <div className="text-[11px] font-bold uppercase tracking-wider text-textMuted px-3 py-1.5 mb-1">
                      Choose Speed & Model
                    </div>
                    {modelOptions.map((opt) => (
                      <React.Fragment key={opt.id}>
                        {opt.dividerBefore && <div className="my-1.5 border-t border-border/40" />}
                        <button
                          onClick={() => {
                            setSelectedModel(opt.id);
                            setShowModelMenu(false);
                          }}
                          className={`flex items-start justify-between w-full p-3 rounded-2xl transition-all text-left group ${
                            selectedModel === opt.id 
                              ? 'bg-primary/10 dark:bg-white/10 text-textMain' 
                              : 'hover:bg-black/5 dark:hover:bg-white/5 text-textMain/80'
                          }`}
                        >
                          ...
                          {opt.badge && (
                            <span className="px-2 py-0.5 rounded-full bg-gray-200 dark:bg-zinc-800 text-[10px] font-bold text-textMain shadow-sm">
                              {opt.badge}
                            </span>
                          )}
                        </button>
              ...
              {/* Speed indicator badge on right */}
              <div className="text-[11px] font-medium text-textMuted flex items-center gap-1.5 px-2">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                <span>{currentModelObj.description}</span>
              </div>
            </div>
            ...
            <div className="flex items-end gap-2 pt-1">
              ...
              <textarea
                ...
                placeholder={`Ask ${currentModelObj.name} anything or trigger an agent...`}
                className="flex-1 bg-transparent text-textMain placeholder-textMuted resize-none outline-none max-h-32 min-h-[44px] py-3 text-[13.5px] leading-relaxed"
                rows={2}
              />
